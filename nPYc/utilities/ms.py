...
def inferBatches(sampleMetadataDf, gapLength=12):
	"""
	...
	"""
	sampleMetadata = sampleMetadataDf.copy()
	...
	if 'Run Order' not in sampleMetadata.columns or 'Acquired Time' not in sampleMetadata.columns:
		...
		currentBatch = 0
		# Loop over samples in run order
		for index, row in sortedSampleMetadata.iterrows():
			if timeDelta > gapLength:
				currentBatch += 1
			sampleMetadata.loc[index, 'Batch'] = currentBatch
			sampleMetadata.loc[index, 'Correction Batch'] = currentBatch
